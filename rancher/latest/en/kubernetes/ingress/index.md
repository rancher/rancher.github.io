...
The Kubernetes Ingress resource can be backed up by any load balancer of your choice, so to leverage the Rancher's load balancing functionality, we have introduced the concept of a Rancher ingress controller, which is automatically created whenever an Ingress is added to Kubernetes.

### Current Limitations 

* Ingress support can only be added through `kubectl`
* 1 Host in the Kubernetes environment must have port `80` available as currently our ingress controller uses this port
...
Using `kubectl`, let's launch our nginx service into Rancher. Remember, you can either [configure `kubectl` for your local machine]({{site.baseurl}}/rancher/{{page.version}}/{{page.lang}}/kubernetes/#kubectl) or you can use the shell in the UI under **Kubernetes** -> **kubectl**.
...
Let's create the ingress using `kubectl`. After you create the ingress, a load balancer will be created and visible in the **kubernetes-ingress-lbs** stack within the **Kubernetes** -> **System** tab. From `kubectl`, you can get the ingress created, but the UI will only show the load balancer, which has translated the requests in the ingress to a Rancher load balancer.  
...
If there are multiple services in the ingress, then you could add host/path based routing to your ingress to balance between the services. 
...
Let's add our services and replication controllers into Kubernetes.
...
Let's launch our ingress, which will start up a load balancer that will have hostname routing rules applied for the multiple services. 
