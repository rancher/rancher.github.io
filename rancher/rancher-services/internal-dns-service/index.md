...
In the UI, when [adding a service]({{site.baseurl}}/rancher/rancher-ui/applications/stacks/adding-services/), expand the **Service Links** section, select the service, and provide the alias name.

If you're using `rancher-compose` to [add the service]({{site.baseurl}}/rancher/rancher-compose/), the `docker-compose.yml` would use either the `links` or `external_links` directive.
