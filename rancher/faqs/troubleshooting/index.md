...
If your host has a new IP after a reboot, the IP of your host in Rancher will no longer match. Your containers will no longer have access to the managed network. To get the host and all containers back into the managed network, just re-run the command to [add in a custom host]({{site.baseurl}}/rancher/rancher-ui/infrastructure/hosts/custom/) and edit the command to specify the new IP. Do not stop or remove the existing agent on the host!
