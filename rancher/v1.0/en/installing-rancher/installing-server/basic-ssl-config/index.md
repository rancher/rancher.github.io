...
```bash
$ sudo docker run -d --restart=always --name=rancher-server rancher/server:v1.0.2
```
<br>
...
1. Launch the Rancher server container with the modified Docker command. The certificate **must** be called `ca.crt` inside the container. 
   ...
   ```bash
   $ sudo docker run -d --restart=always -p 8080:8080 -v /some/dir/cert.crt:/ca.crt rancher/server:v1.0.2
   ```
    <br>
