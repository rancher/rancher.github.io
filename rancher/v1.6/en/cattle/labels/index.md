...
`io.rancher.container.requested_ip` | A list of IPs from the 10.42.0.0/16 address space | Allows you to pick a specific IP for a container. If you put it into a service the containers inside the service will pick up the ip from the list until they are exhausted. The format is comma-separated, like `10.42.100.100, 10.42.100.101`. Note: If the IP is not available on any host, container will start with a random IP.
