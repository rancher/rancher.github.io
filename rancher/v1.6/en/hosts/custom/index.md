...
* _Hosts in Kubernetes:_ Hosts that will be used as Kubernetes nodes will require the following TCP ports to be open for `kubectl`: `10250` and `10255`. To access any exposed services, the ports used for the NodePort will also need to be opened. The default ports used by NodePort are TCP ports `30000` - `32767`.
